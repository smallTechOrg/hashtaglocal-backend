 ...
 *
 * Phase 1 (Discovery): - POST /admin/discovery/start - Start discovery run -
 * GET /admin/discovery/status/{id} - Check discovery progress
 *
 * Phase 2 (Import): - POST /admin/import/start - Start import job - GET
...
public class LocalityAdminController {
	 ...
	 *
	 * Queries GeoNames, OSM, and deduplicates results for all locality types.
	 */
	...
	public LocalityDiscoveryRun startDiscovery(@RequestParam String countryCode) {
		...
		log.info("✅ Complete discovery finished: totalRaw={} geonames={} osm={}", run.getTotalRawDiscoveries(),
				run.getGeonamesCount(), run.getOsmCount());
		return run;
	}
	...
	public LocalityDiscoveryRun getDiscoveryStatus(@PathVariable Long id) {
		return discoveryRunRepository.findById(id)
	...
	public ImportJob startImport(@RequestParam Long discoveryRunId, @RequestParam String localityType) {
		log.info("🚀 Starting import job for discovery run {}, localityType={}", discoveryRunId, localityType);

		LocalityDiscoveryRun discoveryRun = discoveryRunRepository.findById(discoveryRunId)
				.orElseThrow(() -> new RuntimeException("Discovery run not found: " + discoveryRunId));

		// Get discovered localities matching the type
		var localities = discoveredLocalityRepository.findByDiscoveryRunAndLocalityType(discoveryRun,
				org.smalltech.hashtaglocal_backend.entity.DiscoveredLocality.LocalityType.valueOf(localityType));

		log.info("📥 Found {} localities of type {} to import", localities.size(), localityType);

		// Create import job
		ImportJob job = ImportJob.builder().startedAt(java.time.LocalDateTime.now())
				.status(ImportJob.ImportJobStatus.RUNNING).totalLocalities(localities.size()).build();
		...
		for (var locality : localities) {
			try {
				...
				if (status
						.getImportStatus() == org.smalltech.hashtaglocal_backend.entity.LocalityImportStatus.ImportStatus.SUCCESS) {
					successCount++;
				} else if (status
						.getImportStatus() == org.smalltech.hashtaglocal_backend.entity.LocalityImportStatus.ImportStatus.NO_DATA_FOUND) {
					skippedCount++;
				} else {
					failureCount++;
				}
		...
		log.info("✅ Import job completed: Success={}, Failed={}, Skipped={}", successCount, failureCount, skippedCount);
	...
	public ImportJob getImportStatus(@PathVariable Long id) {
		return importJobRepository.findById(id).orElseThrow(() -> new RuntimeException("Import job not found: " + id));
	}
}
