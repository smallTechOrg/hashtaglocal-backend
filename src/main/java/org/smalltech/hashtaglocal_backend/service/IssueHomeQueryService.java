...
public class IssueHomeQueryService {
	...
	public List<IssueEntity> findNearbyIssues(double lat, double lng, double radiusMeters) {
		...
		List<String> statuses = List.of(IssueStatusModel.OPEN.name(), IssueStatusModel.ONHOLD.name(),
				IssueStatusModel.PENDING.name(), IssueStatusModel.RESOLVED.name());

		return issueRepository.findByStatusInAndCreatedAtAfterAndWithinRadius(statuses, since, lat, lng, radiusMeters);
	}
}
