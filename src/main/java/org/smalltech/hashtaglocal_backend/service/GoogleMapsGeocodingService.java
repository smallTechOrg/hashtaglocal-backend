...
/**
 * Service for interacting with Google Maps Geocoding API. Provides reverse geocoding to get
 * detailed location information from coordinates.
 */
@Service
...
public class GoogleMapsGeocodingService {
  ...
  public LocationMetadataDTO reverseGeocode(double latitude, double longitude) {
    try {
      ...
      if (response == null) {
        log.warn(
            "Null response from Google Maps API for coordinates: ({}, {})", latitude, longitude);
        return null;
      }

      if (response.getStatus() != null && !"OK".equals(response.getStatus())) {
        log.warn(
            "Google Maps API returned status '{}' for coordinates: ({}, {})",
            response.getStatus(),
            latitude,
            longitude);
        return null;
      ...
      if (response.getResults() == null || response.getResults().isEmpty()) {
        log.warn(
            "No geocoding results for coordinates: ({}, {}), status: {}",
            latitude,
            longitude,
            response.getStatus());
        return null;
      ...
      log.error(
          "Error during reverse geocoding for ({}, {}): {}",
          latitude,
          longitude,
          e.getMessage(),
          e);
  ...
  /**
   * Extract structured metadata from Google Maps geocoding result. Ensures all key fields are
   * populated with fallbacks and includes complete Google data.
   */
  private LocationMetadataDTO extractMetadata(GoogleMapsLocationDTO.Result result) {
    ...
    return LocationMetadataDTO.builder()
        ...
        .district(district)
        .timezone(null) // Google Maps
        // Geocoding API
        // doesn't
        // provide
        // timezone
        .subRegion(components.get("administrative_area_level_2"))
        .postalCode(components.get("postal_code"))
        .streetNumber(components.get("street_number"))
        .isoCountryCode(components.get("country_code"))
        .formattedAddress(buildFormattedAddress(components, result.getFormattedAddress()))
        .neighborhood(neighborhood)
        .pointOfInterest(pointOfInterest)
        .premiseName(premise)
        .establishmentType(establishment)
        .googleMapsData(buildGoogleMapsDataMap(result, components))
        .addressComponents(new HashMap<>(components))
        .build();
  }

  /**
   * Parse address components into a map for easy access. Handles multiple types for each component
   * and captures all relevant data.
   */
  private Map<String, String> parseAddressComponents(
      List<GoogleMapsLocationDTO.AddressComponent> addressComponents) {
  ...
  /**
   * Extract a meaningful location name from components. Format: "street_number + street -
   * area_name" or just "area_name" if no street. Never include city name in the result.
   */
  private String extractLocationName(
      Map<String, String> components, String street, String city, String district) {
  ...
  /**
   * Build a comprehensive formatted address using key components. Ensures address is always
   * populated with the best available information.
   */
  private String buildFormattedAddress(
      Map<String, String> components, String fullFormattedAddress) {
    String streetNumber = components.get("street_number");
    String route = components.get("route");
    String sublocality2 = components.get("sublocality_level_2");
    String sublocality1 = components.get("sublocality_level_1");
  ...
  private Map<String, Object> buildGoogleMapsDataMap(
      GoogleMapsLocationDTO.Result result, Map<String, String> components) {
