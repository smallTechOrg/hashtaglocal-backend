...
public class GetProfileService {
	...
	public Optional<UserProfileModel> getMyProfile(String accessToken) {
		...
		if (session.getAccessTokenExpiryTs() != null && session.getAccessTokenExpiryTs() < System.currentTimeMillis() / 1000) {
			return Optional.empty();
		}
	...
	private UserProfileModel mapToProfileModel(UserEntity user) {
		return UserProfileModel.builder().username(user.getUsername()).picture(user.getProfilePicture()).hashtag(user.getHashtag()).build();
	}
}
