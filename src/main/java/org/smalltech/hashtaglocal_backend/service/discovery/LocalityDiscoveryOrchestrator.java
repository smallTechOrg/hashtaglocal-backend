 ...
 *
 * <p>Phase 1: Discovery 1. Create discovery run 2. Query GeoNames, OSM, IndiaPost 3. Save all raw
 * discoveries 4. Deduplicate and score 5. Save deduplicated results
 */
@Service
...
public class LocalityDiscoveryOrchestrator {
  ...
  private int saveRawDiscoveries(LocalityDiscoveryRun run, List<RawDiscoveryDTO> discoveries) {
    for (RawDiscoveryDTO dto : discoveries) {
      RawLocalityDiscovery raw =
          RawLocalityDiscovery.builder()
              .discoveryRun(run)
              .source(RawLocalityDiscovery.DiscoverySource.valueOf(dto.getSource()))
              .name(dto.getName())
              .state(dto.getState())
              .countryCode(dto.getCountryCode())
              .localityType(RawLocalityDiscovery.LocalityType.valueOf(dto.getLocalityType()))
              .sourceMetadata(dto.getSourceMetadata())
              .build();
  ...
  private void deduplicateAndSave(LocalityDiscoveryRun run) {
    ...
    Map<RawLocalityDiscovery.LocalityType, Integer> typePriority =
        Map.of(
            RawLocalityDiscovery.LocalityType.CITY,
            1,
            RawLocalityDiscovery.LocalityType.TOWN,
            2,
            RawLocalityDiscovery.LocalityType.DISTRICT,
            3,
            RawLocalityDiscovery.LocalityType.UNKNOWN,
            4);
    ...
    for (DeduplicationEntry entry : dedupMap.values()) {
      DiscoveredLocality locality =
          DiscoveredLocality.builder()
              .discoveryRun(run)
              .officialName(entry.officialName)
              .alternateNames(new ArrayList<>(entry.alternateNames))
              .state(entry.state)
              .countryCode(entry.countryCode)
              .localityType(DiscoveredLocality.LocalityType.valueOf(entry.localityType.toString()))
              .sources(new ArrayList<>(entry.sources))
              .confidenceScore(entry.sources.size())
              .build();
