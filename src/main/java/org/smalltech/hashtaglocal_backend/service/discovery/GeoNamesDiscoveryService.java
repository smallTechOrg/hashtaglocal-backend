 ...
 *
 * Feature codes: - PPLA: State capital (cities) - PPLA2: District capital
 * (towns) - ADM2: Administrative division level 2 (districts)
 */
@Service
...
public class GeoNamesDiscoveryService {
	...
	private List<RawDiscoveryDTO> fetchAllPages(String countryCode, String featureCode, String localityType) {
		...
		try {
			while (startRow < TOTAL_RECORDS_LIMIT) {
				...
				String url = String.format("%s?featureCode=%s&country=%s&maxRows=%d&startRow=%d&username=%s",
						GEONAMES_API_URL, featureCode, countryCode, MAX_ROWS_PER_REQUEST, startRow, geonamesUsername);
				...
				geonamesArray.forEach(node -> {
					RawDiscoveryDTO dto = RawDiscoveryDTO.builder().name(node.get("name").asText())
							.state(node.get("adminName1").asText()).countryCode(countryCode).localityType(localityType)
							.source("GEONAMES").sourceMetadata(buildSourceMetadata(node)).build();
					discoveries.add(dto);
				});

				log.info("📊 Batch fetched {} records (total so far: {}/{})", geonamesArray.size(), discoveries.size(),
						totalResults);
