...
/**
 * Queries OpenStreetMap Overpass API for Indian cities, towns, and districts. With generous timeout
 * (60s) for large queries.
 */
@Service
...
public class OSMDiscoveryService {
  ...
  public List<RawDiscoveryDTO> discoverDistricts(String countryCode) {
    ...
    return queryOverpassWithRetry(
        "[out:json][timeout:60];"
            + "area[\"ISO3166-1\"=\"IN\"][admin_level=2];"
            + "(relation[boundary=administrative][admin_level=5](area););"
            + "out center;",
        "DISTRICT",
        3);
  }

  private List<RawDiscoveryDTO> queryOverpassWithRetry(
      String query, String localityType, int maxRetries) {
    int retries = 0;
    while (retries < maxRetries) {
      try {
        ...
        if (retries < maxRetries) {
          ...
          log.warn(
              "⏳ OSM query timeout for {} (attempt {}/{}). Retrying in {}ms...",
              localityType,
              retries,
              maxRetries,
              waitTime);
          ...
          log.error(
              "❌ OSM {} query failed after {} retries: {}",
              localityType,
              maxRetries,
              e.getMessage());
          return new ArrayList<>();
              ...
              if (name != null && !name.isEmpty() && !name.equals("India")) {
                RawDiscoveryDTO dto =
                    RawDiscoveryDTO.builder()
                        ...
                        .source("OSM")
                        .sourceMetadata(buildSourceMetadata(element))
                        .build();
                discoveries.add(dto);
              }
            });
      ...
      return objectMapper.writeValueAsString(
          new Object() {
            ...
            public final String adminLevel =
                tags != null ? tags.get("admin_level").asText() : "N/A";
          });
    } catch (Exception e) {
