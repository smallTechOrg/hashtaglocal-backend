 ...
 *
 * <p>Phase 2 of the workflow: 1. Validate locality with Google Maps Geocoding API 2. Extract
 * polygon from viewport 3. Save to localities table 4. Track import status with retry logic
 */
@Service
...
public class LocalityImportService {
  ...
  private static final String GOOGLE_MAPS_GEOCODING_URL =
  ...
  public LocalityImportStatus validateAndImport(ImportJob job, DiscoveredLocality discovered) {
    LocalityImportStatus status =
        LocalityImportStatus.builder()
            .importJob(job)
            .discoveredLocality(discovered)
            .localityName(discovered.getOfficialName())
            .state(discovered.getState())
            .localityType(
                LocalityImportStatus.LocalityType.valueOf(discovered.getLocalityType().toString()))
            .attemptCount(1)
            .build();
    ...
    try {
      ...
      // Build query address
      String query =
          String.format("%s, %s, India", discovered.getOfficialName(), discovered.getState());
      ...
      if (geometry != null && geometry.getViewport() != null) {
        ...
        if (ne != null && sw != null) {
          ...
          Coordinate[] coords =
              new Coordinate[] {
                new Coordinate(sw.getLng(), sw.getLat()),
                new Coordinate(ne.getLng(), sw.getLat()),
                new Coordinate(ne.getLng(), ne.getLat()),
                new Coordinate(sw.getLng(), ne.getLat()),
                new Coordinate(sw.getLng(), sw.getLat()) // Close
                // polygon
              };
          ...
          Locality locality =
              Locality.builder()
                  .hashtag(hashtag)
                  .name(discovered.getOfficialName())
      ...
      log.info(
          "✅ Successfully imported {} with hashtag {}",
          discovered.getOfficialName(),
