	...
	public Long handle(Long issueId, Long userId, IssueVerifyRequest request) {
		...
		if (issueActionModel == IssueActionModel.VERIFY || issueActionModel == IssueActionModel.RESOLVE) {
			...
			for (var mediaRequest : mediaUrls) {
				...
				var mediaLocation = locationService.createAndSaveLocation(mediaLocReq.getLat(), mediaLocReq.getLng(),
						mediaLocReq.getMetaData(), "Unknown");

				var mediaEntity = org.smalltech.hashtaglocal_backend.entity.MediaEntity.builder().issue(issueEntity)
						.type(EnumParsers.parseMediaType(mediaRequest.getType())).url(mediaRequest.getUrl())
						.user(userEntity).description(mediaRequest.getDescription()).location(mediaLocation)
