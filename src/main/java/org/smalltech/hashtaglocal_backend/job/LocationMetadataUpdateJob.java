...
/**
 * Job to update location metadata and names using Google Maps Geocoding API. Processes locations in
 * batches to update missing or incomplete metadata.
 */
@Service
...
public class LocationMetadataUpdateJob {
  ...
  public LocationUpdateJobResult runPartialUpdate() {
    ...
    List<Location> locationsToUpdate =
        allLocations.stream()
            .filter(
                loc ->
                    loc.getMetaData() == null
                        || loc.getMetaData().isEmpty()
                        || !hasCompleteMetadata(loc.getMetaData()))
            .toList();

    log.info(
        "📊 Found {} locations with incomplete metadata out of {} total",
  ...
  private LocationUpdateJobResult processLocations(List<Location> locations) {
    ...
    for (int i = 0; i < locations.size(); i++) {
      ...
      try {
        ...
        log.debug(
            "Processing location {}/{}: ID={}, lat={}, lng={}",
            i + 1,
            locations.size(),
            location.getId(),
            latitude,
    ...
    LocationUpdateJobResult result =
        new LocationUpdateJobResult(locations.size(), successCount, failureCount, skippedCount);
  ...
  public record LocationUpdateJobResult(
      int totalProcessed, int successCount, int failureCount, int skippedCount) {
    @Override
