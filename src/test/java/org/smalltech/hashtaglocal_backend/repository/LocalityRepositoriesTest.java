 ...
 *
 * Tests the complete flow: 1. Discovery run created and tracked 2. Raw
 * discoveries saved from multiple sources 3. Discovered localities deduplicated
 * 4. Import jobs and status tracking
 *
 * This is a high-level test showing how to debug the entire workflow in
 * isolation.
 */
@DataJpaTest
@ActiveProfiles("test")
...
class LocalityRepositoriesTest {
	...
	@BeforeEach
	void setUp() {
		run = LocalityDiscoveryRun.builder().countryCode("IN").status(LocalityDiscoveryRun.DiscoveryStatus.IN_PROGRESS)
				.startedAt(LocalDateTime.now()).totalRawDiscoveries(0).geonamesCount(0).osmCount(0).indiaPostCount(0)
				.build();
		run = discoveryRunRepository.save(run);

		job = ImportJob.builder().startedAt(LocalDateTime.now()).totalLocalities(100)
	...
	class RawDiscoveryWorkflow {
		...
		void collectFromMultipleSources() {
			...
			RawLocalityDiscovery geonames = RawLocalityDiscovery.builder().discoveryRun(run)
					.source(RawLocalityDiscovery.DiscoverySource.GEONAMES).name("Mumbai").state("Maharashtra")
					.countryCode("IN").localityType(RawLocalityDiscovery.LocalityType.CITY)
					.sourceMetadata("{\"feature_code\": \"PPLA\"}").build();

			// Simulate OSM discovery (same locality, variant name)
			RawLocalityDiscovery osm = RawLocalityDiscovery.builder().discoveryRun(run)
					.source(RawLocalityDiscovery.DiscoverySource.OSM).name("Bombay").state("Maharashtra")
					.countryCode("IN").localityType(RawLocalityDiscovery.LocalityType.CITY)
					.sourceMetadata("{\"osm_id\": 1234}").build();
			...
			// Query by source
			List<RawLocalityDiscovery> geonamesResults = rawDiscoveryRepository
					.findBySource(RawLocalityDiscovery.DiscoverySource.GEONAMES);
	...
	class DeduplicationWorkflow {
		...
		void deduplicateWithConfidenceScore() {
			...
			RawLocalityDiscovery raw1 = RawLocalityDiscovery.builder().discoveryRun(run)
					.source(RawLocalityDiscovery.DiscoverySource.GEONAMES).name("Bangalore").state("Karnataka")
					.countryCode("IN").localityType(RawLocalityDiscovery.LocalityType.CITY).sourceMetadata("{}")
					.build();

			RawLocalityDiscovery raw2 = RawLocalityDiscovery.builder().discoveryRun(run)
					.source(RawLocalityDiscovery.DiscoverySource.OSM).name("Bengaluru").state("Karnataka")
					.countryCode("IN").localityType(RawLocalityDiscovery.LocalityType.CITY).sourceMetadata("{}")
					.build();
			...
			DiscoveredLocality discovered = DiscoveredLocality.builder().discoveryRun(run).officialName("Bengaluru")
					.alternateNames(Arrays.asList("Bangalore", "Bangaluru")).state("Karnataka").countryCode("IN")
					.localityType(DiscoveredLocality.LocalityType.CITY).sources(Arrays.asList("geonames", "osm"))
		...
		void filterByLocalityType() {
			// Create mixed types
			DiscoveredLocality city = DiscoveredLocality.builder().discoveryRun(run).officialName("Mumbai")
					.alternateNames(Arrays.asList()).state("Maharashtra").countryCode("IN")
					.localityType(DiscoveredLocality.LocalityType.CITY).sources(Arrays.asList("geonames"))
					.confidenceScore(1).build();

			DiscoveredLocality town = DiscoveredLocality.builder().discoveryRun(run).officialName("Aurangabad")
					.alternateNames(Arrays.asList()).state("Maharashtra").countryCode("IN")
					.localityType(DiscoveredLocality.LocalityType.TOWN).sources(Arrays.asList("geonames"))
					.confidenceScore(1).build();
			...
			// Query only cities
			List<DiscoveredLocality> cities = discoveredLocalityRepository
					.findByLocalityType(DiscoveredLocality.LocalityType.CITY);
			assertTrue(cities.stream().allMatch(l -> l.getLocalityType() == DiscoveredLocality.LocalityType.CITY));
		}
	}
	...
	class ImportValidationWorkflow {
		...
		void trackImportJobProgress() {
			...
			DiscoveredLocality discovered = DiscoveredLocality.builder().discoveryRun(run).officialName("Pune")
					.alternateNames(Arrays.asList()).state("Maharashtra").countryCode("IN")
					.localityType(DiscoveredLocality.LocalityType.CITY).sources(Arrays.asList("geonames"))
					.confidenceScore(1).build();
			DiscoveredLocality saved = discoveredLocalityRepository.save(discovered);
			...
			LocalityImportStatus successStatus = LocalityImportStatus.builder().importJob(job).discoveredLocality(saved)
					.localityName("Pune").localityType(LocalityImportStatus.LocalityType.CITY).state("Maharashtra")
					.importStatus(LocalityImportStatus.ImportStatus.SUCCESS).attemptCount(1).build();
		...
		void trackRetryLogic() {
			DiscoveredLocality discovered = DiscoveredLocality.builder().discoveryRun(run).officialName("Test City")
					.alternateNames(Arrays.asList()).state("Test State").countryCode("IN")
					.localityType(DiscoveredLocality.LocalityType.CITY).sources(Arrays.asList("geonames"))
					.confidenceScore(1).build();
			DiscoveredLocality saved = discoveredLocalityRepository.save(discovered);

			// First attempt failed
			LocalityImportStatus failed = LocalityImportStatus.builder().importJob(job).discoveredLocality(saved)
					.localityName("Test City").localityType(LocalityImportStatus.LocalityType.CITY).state("Test State")
					.importStatus(LocalityImportStatus.ImportStatus.FAILED).errorMessage("API rate limit exceeded")
