...
class MediaIntegrationTests {
	...
	void shouldReturnValidSignedUploadUrlResponse() throws Exception {
		...
		when(storage.signUrl(any(BlobInfo.class), eq(15L), eq(TimeUnit.MINUTES), any(Storage.SignUrlOption.class),
				any(Storage.SignUrlOption.class))).thenReturn(fakeUrl);
		...
		webTestClient.get().uri(SIGNED_URL_API).header("Authorization", "Bearer " + token).exchange().expectStatus()
				.isOk().expectBody().jsonPath("$.data").exists().jsonPath("$.data.media_url").exists()
				.jsonPath("$.data.media_url.signed_url").isEqualTo(fakeUrl.toString()).jsonPath("$.data.media_url.path")
	...
	void shouldFailIfContentTypeMissing() {
		...
		webTestClient.get().uri("/api/v1/media/upload-url").header("Authorization", "Bearer " + token).exchange()
