...
class LocalityIntegrationTest {
  ...
  void testGetAllLocalitiesWithPolygons() {
    ...
    Locality locality1 =
        createAndSaveLocality(
            "test_indiranagar", "Indiranagar", 77.6408, 12.9716, 77.6500, 12.9800);
    Locality locality2 =
        createAndSaveLocality(
            "test_koramangala", "Koramangala", 77.6100, 12.9352, 77.6200, 12.9400);

    // Act & Assert
    webTestClient
        .get()
        .uri(LOCALITIES_POLYGONS_URL)
        .exchange()
        .expectStatus()
        .isOk()
        .expectBody()
        ...
        .isNotEmpty()
        .jsonPath("$[0].hashtag")
        .isEqualTo("test_indiranagar")
        .jsonPath("$[0].name")
        .isEqualTo("Indiranagar")
        .jsonPath("$[0].geoBoundary")
        .isNotEmpty()
        .jsonPath("$[0].geoBoundary.type")
        .isEqualTo("Polygon")
        .jsonPath("$[0].geoBoundary.coordinates")
        .isArray()
        .jsonPath("$[0].geoBoundary.coordinates[0]")
        .isArray()
        .jsonPath("$[0].geoBoundary.coordinates[0].length()")
        .value(greaterThanOrEqualTo(4))
        .jsonPath("$[1].hashtag")
        .isEqualTo("test_koramangala")
        .jsonPath("$[1].name")
        .isEqualTo("Koramangala");
  }
    ...
    webTestClient
        ...
        .value(
            hasItems(
                ...
                "test_locality4",
                "test_locality5"))
        .jsonPath("$[*].geoBoundary.type")
        .value(everyItem(equalTo("Polygon")));
  }

  /** Helper method to create and save a test locality with polygon. */
  private Locality createAndSaveLocality(
      String hashtag, String name, double minLng, double minLat, double maxLng, double maxLat) {
