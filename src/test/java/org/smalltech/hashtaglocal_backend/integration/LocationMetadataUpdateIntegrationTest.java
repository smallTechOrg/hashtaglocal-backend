...
/**
 * Integration test for Location Metadata Update Job. Note: This test requires a valid Google Maps
 * API key to be configured. It will be skipped if the API key is not set or if running in CI.
 */
@SpringBootTest
@ActiveProfiles("test")
...
class LocationMetadataUpdateIntegrationTest {
  ...
  void setUp() {
    ...
    testLocality =
        Locality.builder()
            .hashtag("#test-locality")
            .name("Test Locality")
            .geoBoundary(
                LocationUtil.createPoint(12.9716, 77.5946)
                    .getFactory()
                    .createPolygon(
  ...
  void shouldHandleLocationWithNullMetadata() {
    ...
    Location location =
        Location.builder()
            .point(point)
            .name("Location Without Metadata")
            .locality(testLocality)
            .build();
  ...
  void shouldRetrieveAndVerifyMetadataStructure() {
    ...
    assertEquals(
        "123 MG Road, Bengaluru, Karnataka", retrieved.getMetaData().get("formatted_address"));
  }
}
