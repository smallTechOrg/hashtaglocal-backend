...
class IssueReportIntegrationTests {
	...
	private String createAuthenticatedToken() {
		...
		if (localityRepository.findById(1L).isEmpty()) {
			...
			Coordinate[] coords = new Coordinate[]{new Coordinate(-180, -90), new Coordinate(180, -90),
			...
			org.smalltech.hashtaglocal_backend.entity.Locality worldLocality = org.smalltech.hashtaglocal_backend.entity.Locality
					.builder().hashtag("#world").name("World").geoBoundary(worldPolygon).build();
		...
		UserAuthProviderEntity provider = userAuthProviderRepository
				.save(UserAuthProviderEntity.builder().user(user).providerType("TEST").providerUserId(username)
						.email("test-" + System.currentTimeMillis() + "@example.com").build());
	...
	void createIssue_shouldReturnSuccessResponse() throws Exception {
		...
		webTestClient.post().uri("/api/v1/issue").contentType(MediaType.APPLICATION_JSON)
				.header("Authorization", "Bearer " + token).bodyValue(requestJson).exchange().expectStatus().isOk()
				.expectBody().jsonPath("$.data.issue_id").exists();

		// Assert DB entries - find by description since we know it's unique to this
		// test
		List<IssueEntity> issues = issueRepository.findAll();
		IssueEntity issue = issues.stream().filter(i -> "Test pothole issue integration".equals(i.getDescription()))
				.findFirst().orElseThrow(
						() -> new AssertionError("Issue with description 'Test pothole issue integration' not found"));
		...
		MediaEntity media = mediaList.stream().filter(m -> "https://example.com/test-photo.jpg".equals(m.getUrl()))
				.findFirst()
				.orElseThrow(() -> new AssertionError("Media with URL 'https://example.com/test-photo.jpg' not found"));
