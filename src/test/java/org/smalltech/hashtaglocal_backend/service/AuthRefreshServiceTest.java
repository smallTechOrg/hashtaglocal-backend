...
class AuthRefreshServiceTest {
	...
	private UserAuthSessionEntity createActiveSession(String accessToken, long accessTokenExpiry, String refreshToken,
	...
	void testRefreshTokens_WithValidRefreshToken_AndExpiredAccessToken_ShouldGenerateNewTokens() {
		// Arrange
		UserAuthSessionEntity session = createActiveSession(ACCESS_TOKEN, PAST_TIME, // access token expired
				REFRESH_TOKEN, FUTURE_TIME); // refresh token valid

		when(userAuthSessionRepository.findByRefreshToken(REFRESH_TOKEN)).thenReturn(Optional.of(session));
	...
	void testRefreshTokens_WithValidRefreshToken_AndActiveAccessToken_ShouldReturnExistingTokens() {
		// Arrange
		UserAuthSessionEntity session = createActiveSession(ACCESS_TOKEN, FUTURE_TIME, // access token still active
				REFRESH_TOKEN, FUTURE_TIME); // refresh token valid
	...
	void testRefreshTokens_WithExpiredRefreshToken_ShouldThrowException() {
		// Arrange
		UserAuthSessionEntity session = createActiveSession(ACCESS_TOKEN, PAST_TIME, // access token expired
				REFRESH_TOKEN, PAST_TIME); // refresh token also expired
	...
	void testRefreshTokens_WithInactiveSession_ShouldThrowException() {
		// Arrange
		UserAuthSessionEntity session = createActiveSession(ACCESS_TOKEN, FUTURE_TIME, REFRESH_TOKEN, FUTURE_TIME);
	...
	void testRefreshTokens_SessionValidationOrder_InactiveSessionBeforeRefreshTokenExpiry() {
		...
		UserAuthSessionEntity session = createActiveSession(ACCESS_TOKEN, FUTURE_TIME, REFRESH_TOKEN, PAST_TIME); // refresh
																													// token
																													// expired
	...
	void testRefreshTokens_UpdatesSessionWhenGeneratingNewTokens() {
		...
		verify(userAuthSessionRepository)
				.save(argThat(savedSession -> savedSession.getAccessToken().equals(newAccessToken)
