...
class IssueImportServiceTest {
  ...
  @Mock private IssueImportJobRepository jobRepository;

  @Mock private IssueImportStatusRepository statusRepository;
  ...
  void setUp() {
    ...
    when(jobRepository.save(any(IssueImportJob.class)))
        .thenAnswer(
            invocation -> {
    ...
    when(statusRepository.save(any(IssueImportStatus.class)))
  ...
  void importsBlrPagesIssues() {
    BlrPagesIssueDTO dto =
        BlrPagesIssueDTO.builder()
            .uuid("test-uuid-1")
            .lat(12.9)
            .lng(77.6)
            .image("https://example.com/photo.jpg")
            .category(3)
            .createdAt("2025-06-17T17:21:24.077Z")
            .build();
    ...
    Locality locality =
        Locality.builder()
            ...
            .geoBoundary(null)
            .build();
    when(localityRepository.findByHashtag("#bengaluru"))
    ...
    when(restTemplate.exchange(
            eq(dto.getImage()), eq(HttpMethod.GET), any(HttpEntity.class), eq(byte[].class)))
        .thenReturn(download);
