...
```bash
gcloud compute scp build/libs/hashtaglocal-backend-0.0.1-SNAPSHOT.jar staging-instance:/tmp/hashtaglocal-backend.jar --zone=us-central1-c

gcloud compute ssh staging-instance --zone=us-central1-c --command="sudo mv /tmp/hashtaglocal-backend.jar /opt/hashtaglocal-backend/hashtaglocal-backend.jar && sudo systemctl restart hashtaglocal-backend" 

gcloud compute ssh staging-instance --zone=us-central1-c \
  --command="sudo journalctl -u hashtaglocal-backend -f"
